...
def assert_sql_data(environment, db, field, table, factor_str, except_result, is_equal):
    """
    断言数据库查询数据
...
def assert_api(except_rule, except_result, environment="test_hk"):
    ...
    except_method = except_rule_list[0]
    rule = except_rule_list[1]
    is_equal = except_rule_list[2]
    if except_method == "res":
        return assert_res_data(rule, except_result, is_equal)
    elif except_method == "sql":
        rule_sql = rule.split("=")
        db = rule_sql[0]
        field = rule_sql[1]
        table = rule_sql[2]
        factor_str = rule_sql[3]
        return assert_sql_data(environment, db, field, table, factor_str, except_result, is_equal)
    else:
